...
public class CarFleet {
    public static void main(String[] args) {
        ...
        int[] speed = {2, 2, 1, 1};
        int target = 10;
        System.out.println(numberOfCarFleets(position, speed, target));
    }

    static int numberOfCarFleets(int[] position, int[] speed, int target) {
        ...
        Arrays.sort(cars, Comparator.comparingDouble((double[] car) -> car[0]).reversed());
        Stack<Double> stack = new Stack<>();

        for (double[] car: cars) {
            double endTime = car[1];
            if(stack.isEmpty() || endTime > stack.peek()) {
                stack.push(endTime);
            }
        }
        return stack.size();
